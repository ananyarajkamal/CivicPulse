...
def upgrade() -> None:
    source_enum = sa.Enum('web', 'whatsapp_demo', 'social_demo', 'municipal_demo', name='source_enum')
    source_enum.create(op.get_bind(), checkfirst=True)
    op.add_column(
        'complaints',
        sa.Column(
            'source',
            sa.Enum('web', 'whatsapp_demo', 'social_demo', 'municipal_demo', name='source_enum'),
            server_default='web',
            nullable=False,
        )
    )

...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "default_sla_hours", sa.Integer(), nullable=False, server_default="48"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "default_priority", priority_enum, nullable=False, server_default="medium"
        ),
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
        sa.CheckConstraint(
            "role != 'municipal_officer' OR department_id IS NOT NULL",
            name="ck_officer_department_required",
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "category_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("complaint_categories.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "department_id",
        ...
        sa.Column(
            "is_safety_risk", sa.Boolean(), nullable=False, server_default="false"
        ),
    ...
    op.create_index(
        "idx_complaints_tracking_id", "complaints", ["tracking_id"], unique=True
    )
    ...
    op.create_index(
        "idx_complaints_created_at", "complaints", [sa.text("created_at DESC")]
    )
    ...
    op.create_index(
        "idx_status_history_complaint", "complaint_status_history", ["complaint_id"]
    )
    ...
    op.execute(
        "CREATE POLICY departments_admin_all ON departments FOR ALL USING (auth.jwt() ->> 'role' = 'admin');"
    )

    # 2. complaint_categories policies
    op.execute(
        "CREATE POLICY categories_public_select ON complaint_categories FOR SELECT USING (true);"
    )
    op.execute(
        "CREATE POLICY categories_admin_all ON complaint_categories FOR ALL USING (auth.jwt() ->> 'role' = 'admin');"
    )
    ...
    op.execute(
        "CREATE POLICY complaints_public_select ON complaints FOR SELECT USING (tracking_id IS NOT NULL);"
    )
    op.execute(
    ...
    op.execute(
        "CREATE POLICY ai_logs_admin_all ON ai_processing_logs FOR ALL USING (auth.jwt() ->> 'role' = 'admin');"
    )

    # 8. refresh_tokens policies
    op.execute(
        "CREATE POLICY refresh_tokens_service_all ON refresh_tokens FOR ALL USING (auth.jwt() ->> 'role' = 'service_role' OR auth.jwt() ->> 'role' = 'admin');"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP POLICY IF EXISTS status_history_officer_select ON complaint_status_history;"
    )
    ...
    op.execute(
        "DROP POLICY IF EXISTS categories_public_select ON complaint_categories;"
    )
